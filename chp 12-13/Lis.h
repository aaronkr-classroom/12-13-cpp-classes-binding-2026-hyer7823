...
public:
	Nodeltr& operator++(); // 한 개씩 증가 연산자
	Nodeltr& operator--(); // 감소 연산자
...
template <class T> Nodeltr<T>& Nodeltr<T>::operator++() {
	node = node->next;
	return *this;
}
...
template <class T> Nodeltr<T> Nodeltr<T>::operator++(int) {
	Nodeltr<T> ret(node);
	node = node->next;
	return *this;
}
